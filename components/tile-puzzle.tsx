...
import { ArrowLeft, ArrowRight, Check, CheckCircle2, Clock3, Eye, EyeOff, Gamepad2, Hash, Move, Pause, Play, Shuffle, Trophy } from "lucide-react";
      ...
      <main id="puzzle-main" className="puzzle-main">
        ...
        <div className="puzzle-layout">
          <section className="puzzle-arena" aria-labelledby="puzzle-heading">
            ...
            <div className="puzzle-controls"><button type="button" onClick={() => requestNew({ pokemon: selected.id, size })} disabled={!ready}><Shuffle size={16} /> New shuffle</button><button type="button" onClick={() => paused ? begin() : pauseGame()} disabled={!ready || won}>{paused ? <Play size={16} /> : <Pause size={16} />}{paused ? "Resume" : "Pause"}</button><button type="button" aria-pressed={showNumbers} onClick={() => setShowNumbers(!showNumbers)}><Hash size={16} /> Numbers</button><button type="button" aria-expanded={showReference} aria-controls="puzzle-reference" onClick={() => setShowReference(!showReference)}>{showReference ? <EyeOff size={16} /> : <Eye size={16} />} Reference</button></div>
            ...
            <div id="puzzle-reference" className="puzzle-reference" hidden={!showReference}><div><span className="puzzle-overline">THE FINISHED PICTURE</span><h3>{selected.name}</h3><p>A little nudge in the right direction.</p></div><img src={image} alt={`Complete ${selected.name} picture to match`} width={220} height={220} style={{ backgroundImage: backdrop }} onError={() => setImageFailed(true)} /></div>
